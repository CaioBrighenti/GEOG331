...
viewRGB(rgbS,r=1,g=2,b=3,maxpixels =  2297430,quantiles = c(0.00,0.995), homebutton=FALSE,
  ...
  mapview(agri, color="grey25",col.regions="violet")+mapview(built, color="grey25",col.regions="darkgoldenrod3")+
...
pred_comp <- abs(nnet_prediction - rf_prediction)
plot(pred_comp,
     breaks=seq(0,6), 
     col=landclass$cols ,
     legend=FALSE, axes=FALSE)
#add legend
legend("bottomleft", paste(seq(0,5)),
       fill=landclass$cols, bty="n")   


####### QUESTION 6
## extract needed values
nn_refs <- colSums(nn_errorM$table)
nn_preds <- rowSums(nn_errorM$table)
nn_accpreds <- diag(nn_errorM$table)

## extract needed values
rf_refs <- colSums(rf_errorM$table)
rf_preds <- rowSums(rf_errorM$table)
rf_accpreds <- diag(rf_errorM$table)

# prod error
# producer's accuracy = the number of reference sites classified accurately 
# divided by the total number of reference sites for that class
nn_prods <- nn_accpreds / nn_refs 
rf_prods <- rf_accpreds / rf_refs

# user error
# The User's Accuracy is calculating by taking the total number of correct 
# classifications for a particular class and dividing it by the row total.
nn_users <- nn_accpreds / nn_preds
rf_users <- rf_accpreds / rf_preds
